...
void CanvasLayer::OnAttach()
{
	if (!m_CanvasImage)
	{
		...
		m_ColourFillShader = std::make_unique<Stylus::ComputePipeline>(m_CanvasImage, "assets/shaders/fill-colour.spv", sizeof(Stylus::ColourFillPushData));

		Stylus::ColourFillPushData pushData{};
		pushData.Colour = glm::vec4(1.0f, 1.0f, 1.0f, 1.0f);
		m_ColourFillShader->DispatchShader(m_CanvasImage->GetWidth(), m_CanvasImage->GetHeight(), &pushData);
	}
}
...
void CanvasLayer::OnUIRender()
{
	...
	if (ImGui::IsMouseDown(0))
	{
		...
		m_ToolShader->DispatchShader(m_CanvasImage->GetWidth(), m_CanvasImage->GetHeight(), &pushData);
	}
